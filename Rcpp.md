...
add
#> function (x, y, z) 
#> .Call(<pointer: 0x7f9eb8e55f60>, x, y, z)
add(1, 2, 3)
#> [1] 6
```
...
#> 1 sum(x)       1.31µs   1.52µs   1.33µs   76.8µs   658975.
#> 2 sumC(x)      3.24µs   4.66µs   4.75µs      1ms   214615.
#> 3 sumR(x)     38.24µs  40.45µs  39.05µs    148µs    24722.
```
...
#>   expression          min     mean   median      max `itr/sec`
#>   <chr>          <bch:tm> <bch:tm> <bch:tm> <bch:tm>     <dbl>
#> 1 pdistR(0.5, y)   8.64ms   9.17ms    9.1ms   12.4ms      109.
#> 2 pdistC(0.5, y)   4.75ms   4.97ms   4.88ms   6.48ms      201.
```
...
#> 1 gibbs_r(1…   5.65ms   5.92ms   5.78ms 9.35ms      169.    4.97MB
#> 2 gibbs_cpp… 308.08µs 356.66µs 347.83µs 1.47ms     2804.     4.1KB
...
#> # A tibble: 3 x 10
#>   expression      min    mean   median      max `itr/sec` mem_alloc
#>   <chr>      <bch:tm> <bch:t> <bch:tm> <bch:tm>     <dbl> <bch:byt>
#> 1 vacc1        1.85ms     2ms   1.95ms   5.54ms      500.    7.86KB
#> 2 vacc2      101.98µs 121.8µs 110.67µs 334.92µs     8210.  223.98KB
#> 3 vacc3       29.08µs  31.9µs  30.32µs 175.17µs    31334.   14.48KB
#> # … with 3 more variables: n_gc <dbl>, n_itr <int>,
#> #   total_time <bch:tm>
```
