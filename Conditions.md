...
<!-- GVW: I think this is the first time I've seen a trailing '.' on a parameter name - is it significant in any way? -->
...
<!-- GVW: I do not see this behavior in RStudio.  Instead, when I paste in the definition of `fw` and call it (in the same code block, if that matters) I get:
...
<!-- GVW: this section has introduced assignment as an operator inside a function call with `try(default <- read.csv(...), ...)` and now anonymous code blocks `suppressWarnings({...})` without drawing attention to either - maybe link to the sections that explain both? -->
...
<!-- GVW: I don't understand how the example below illustrates this point -->
...
<!-- GVW: "to a file" rather than "to a path" in the above? -->
