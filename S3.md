...
*   Data frames are similar to record style objects in that both use lists of
    ...
    and the individual components are readly exposed to the user. The number of
...
Note that `POSIXt` does not adhere to these principles becase `POSIXct` has type double, and `POSIXlt` has type list. This illustrates that it's quite possible to use the S3 inheritance system to implement other styles of code sharing (here `POSIXt` plays a role more like an interface), but you'll need to figure out safe conventions yourself.
